...
| Shinbo is enabled under Privacy & Security → Accessibility, yet `computer` and ⌥⌥ report that the grant is missing | macOS stores each grant with the code-signing requirement of the build that asked. A locally packaged Shinbo is ad-hoc signed, so its grant is pinned to that one build's hash; the notarized app under the same bundle id fails that requirement, and toggling the switch does not refresh it. | Open Shinbo → Settings → Permissions → Accessibility → **Settings ↗**. When the app is not trusted, `resetStaleAccessibilityGrant` ([main.ts](../desktop/main/main.ts)) runs `tccutil reset Accessibility <bundle id>` before prompting, so macOS asks again and stores the current requirement. On the notarized app that requirement is the Developer ID identity, which survives updates. By hand: `tccutil reset Accessibility com.tronschell.emma`, then relaunch and grant |
