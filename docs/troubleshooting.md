...
| Opening Shinbo after installing a new disk image still shows an older version, or the Accessibility prompt names an old build such as "Emma" | Every copy of the app that Spotlight has seen and shares the shipped bundle id is a candidate when macOS opens Shinbo by identifier (Spotlight, notifications, the Accessibility prompt's name lookup), and it does not prefer `/Applications`. Stale `desktop/release` builds and old Emma builds all claimed `com.tronschell.emma`. | Since 0.9.3 local `package:mac` builds use `com.tronschell.shinbo.dev` unless `SHINBO_BUNDLE_ID` says otherwise, and a packaged copy that starts while a newer Shinbo sits in `/Applications` offers to open that one instead (`handOffToInstalledShinbo` in [main.ts](../desktop/main/main.ts)). Delete or rebuild any pre-0.9.3 bundles under `desktop/release` and any old Emma checkouts; Spotlight re-registers them as long as they exist |
...
| Problem | Cause | Fix |
| --- | --- | --- |
| ⌥⌥ does nothing on macOS | `NSEvent addGlobalMonitorForEventsMatchingMask` reports other apps' keys only to a trusted process. `shinbo-option-tap` prints `Shinbo: Accessibility access is required to control the computer.` to stderr ([quick_ask.m:613](../desktop/native/quick_ask.m#L613)). | Grant Accessibility. Shinbo polls the grant every three seconds while it is missing and restarts the helper once it appears, so no relaunch is needed |
| Shinbo is enabled under Privacy & Security → Accessibility, yet `computer` and ⌥⌥ report that the grant is missing | macOS stores each grant with the code-signing requirement of the build that asked. A locally packaged Shinbo is ad-hoc signed, so its grant is pinned to that one build's hash; the notarized app under the same bundle id fails that requirement, and toggling the switch does not refresh it. | Since 0.9.4 a packaged Shinbo repairs this itself: at startup, `resetStaleGrants` ([main.ts](../desktop/main/main.ts)) runs `tccutil reset <service> <bundle id>` for Accessibility when the app is not trusted, and for Microphone and Screen Recording when they read as denied, before the Quick Ask helper prompts; a refused `computer` call raises the Accessibility prompt again. macOS then stores the current requirement, which for the notarized app and for a local build signed with the Developer ID identity in your keychain is the team identity that survives updates. Turn Shinbo on when the prompt appears; nothing needs a relaunch. By hand: `tccutil reset Accessibility com.tronschell.emma`, then reopen Shinbo and grant |
...
| `Accessibility permission is required. macOS is asking for it now…` | `shinbo-computer` is not trusted ([computer.m](../desktop/native/computer.m)); Shinbo reset any stale grant at startup and has just raised the system prompt once per minute at most ([main.ts](../desktop/main/main.ts)). The helper process is dropped so the retry starts a fresh one. | Turn Shinbo on under Privacy & Security → Accessibility, then let the agent call `get_app_state` again |
