  ...
  const server = Bun.serve({
    ...
    fetch(request) {
      ...
      return Response.json(
        { error: { code: "credit_card_required", message: "Buy credits to use this provider." } },
        { status: 403 },
      );
    },
  });
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        await session.waitForText("Buy credits to use this provider.", 10_000);
        await session.waitForComposer(10_000);
        ...
        expect(scrollback).toContain("Buy credits to use this provider.");
