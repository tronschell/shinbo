...
test "model cache refetches effective access across auth changes" {
    ...
    const cases = [_]struct { access: credentials.CatalogAccess, model_id: []const u8 }{
        ...
        .{ .access = authenticatedCatalogAccess("private-key"), .model_id = "private/one" },
        .{ .access = public_access, .model_id = "public/again" },
    };
...
test "model cache reloads a ready authenticated catalog after access downgrades" {
    ...
    for (cases) |case| {
        ...
        try std.testing.expect(runtime.catalogModelCompletion("private/only-model") == null);
...
test "model cache warmup publishes a snapshot and filtered completion" {
    ...
    var runtime = Runtime.init(std.testing.allocator, "/v1/models");
    defer runtime.deinit();
    runtime.startWarmup(test_builtin_gateway.model_catalog_provider, authenticatedCatalogAccess("test-key"));
    try waitForWarmup(&runtime);
...
test "model cache reset replaces ready public catalog with private catalog" {
    var runtime = Runtime.init(std.testing.allocator, "/v1/models");
    defer runtime.deinit();
...

    {
        ...
        runtime.startWarmup(test_builtin_gateway.model_catalog_provider, authenticatedCatalogAccess("private-key"));
        try waitForWarmup(&runtime);
