...
pub fn Runtime(comptime App: type) type {
    return struct {
        ...
        fn dismissActiveMenusForComposerEdit(app: *App) bool {
            return dismissSkillsMenuForComposerEdit(app);
        }
...
/// A composer picker parked on its middle row, so a move in either direction
/// is observable.
fn openRoutingPicker(app: *RoutingFakeApp) !void {
    try openRoutingModelMenu(app, &.{ "alpha/one", "beta/two", "gamma/three" });
    app.model_cache.menu.selected_index = 1;
}
...
test "app_input_runtime ctrl+j and ctrl+k navigate visible composer pickers" {
    ...
    for (cases) |case| {
        ...
        try std.testing.expect(app.model_cache.menu.active);
        try std.testing.expectEqual(case.expected_index, app.model_cache.menu.selected_index);
        try std.testing.expectEqualStrings("/", app.input_runtime.edit_state.input.items);
        try std.testing.expectEqual(@as(usize, 1), app.input_runtime.edit_state.cursor);
        try std.testing.expect(app.shell.render_requests.hasReason(.footer));
    }
