...
pub fn loadStartupState(
    ...
    return loadStartupStateFromOwnedWorkspace(alloc, workspace_root, default_model, default_agent_step_limit, null, true);
}
...
pub fn loadStartupStateWithoutCredentials(alloc: Allocator, default_model: []const u8, default_agent_step_limit: usize) !StartupState {
    const workspace_root = try io_mod.realpathAlloc(alloc, ".");
    return loadStartupStateFromOwnedWorkspace(alloc, workspace_root, default_model, default_agent_step_limit, null, false);
}
...
pub fn loadStartupStatus(
    ...
    const auth_status = try auth_runtime.loadStatusSnapshot(alloc);
...
fn loadStartupStateForWorkspace(alloc: Allocator, workspace_root: []const u8, default_model: []const u8, default_agent_step_limit: usize) !StartupState {
    ...
    return loadStartupStateFromOwnedWorkspace(alloc, owned_workspace_root, default_model, default_agent_step_limit, null, false);
}
...
test "startup provider selection falls back to the default model" {
    ...
    const empty = config_runtime.Settings{};
    const fallback = try configuredProviderSelection("default/model", &empty);
    try std.testing.expectEqual(model_provider.ProviderId.gateway, fallback.provider);
