...
pub fn configureFromEnv(
    ...
    if (configured_path.len > 0) {
        configure(alloc, configured_path, initial_cols, initial_rows, app_version_string) catch |err| {
            if (record_requested) return err;
            return;
        };
    } else if (record_requested) {
        try configureAutomatic(alloc, initial_cols, initial_rows, app_version_string);
    } else {
        return;
    }
...
pub fn configure(
    ...
    try configureWithOptions(alloc, path, initial_cols, initial_rows, app_version_string, false, false);
}
...
fn configureWithOptions(
    ...
    const header = buildHeader(initial_cols, initial_rows, app_version_string);
...
fn buildHeader(initial_cols: u16, initial_rows: u16, app_version_string: []const u8) Header {
    var header: Header = .{
        .fixed = undefined,
        .version_tail = app_version_string,
    };
    ...
    idx += 8;
    header.fixed[idx] = @intCast(@min(app_version_string.len, @as(usize, 255)));
    return header;
}
