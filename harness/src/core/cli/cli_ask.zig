...
const AskContext = struct {
    ...
    fn agentStreamProvider(self: *const AskContext) agent_stream_provider.Provider {
        return self.cfg.gateway_provider.agent_stream;
    }
...
fn runPromptInternal(alloc: Allocator, prompt: []const u8, permission_override: ?PermissionMode, cfg: Config, options: RunOptions) !PromptRunResult {
    ...
    const credential: *const credentials.Credential = if (startup_matches_final_model)
        ...
        const resolution = try credentials.resolve(alloc);
...
fn loadStartupStateDefault(
    ...
    return app_lifecycle.loadStartupState(alloc, default_model, default_agent_step_limit);
}
...
fn testMissingKeyStartup(alloc: Allocator, default_model: []const u8, default_agent_step_limit: usize) !app_lifecycle.StartupState {
...
fn testPresentKeyStartup(alloc: Allocator, default_model: []const u8, default_agent_step_limit: usize) !app_lifecycle.StartupState {
    var state = app_lifecycle.StartupState{ .agent_step_limit = default_agent_step_limit };
    errdefer state.deinit(alloc);
    state.workspace_root = try alloc.dupe(u8, "/tmp/fx-test");
    state.credential = .{
...
fn testMissingKeyAcknowledgedStartup(alloc: Allocator, default_model: []const u8, default_agent_step_limit: usize) !app_lifecycle.StartupState {
    var state = try testMissingKeyStartup(alloc, default_model, default_agent_step_limit);
    state.yolo_acknowledged = true;
    return state;
}

fn testMissingKeyDiagnosticStartup(alloc: Allocator, default_model: []const u8, default_agent_step_limit: usize) !app_lifecycle.StartupState {
    var state = try testMissingKeyStartup(alloc, default_model, default_agent_step_limit);
    errdefer state.deinit(alloc);
...
fn testPresentKeySavedStartup(alloc: Allocator, default_model: []const u8, default_agent_step_limit: usize) !app_lifecycle.StartupState {
    var state = try testPresentKeyStartup(alloc, default_model, default_agent_step_limit);
    errdefer state.deinit(alloc);
...
fn testCountImagePreflightStartup(alloc: Allocator, default_model: []const u8, default_agent_step_limit: usize) !app_lifecycle.StartupState {
    test_image_preflight_startup_calls += 1;
    return testPresentKeyStartup(alloc, default_model, default_agent_step_limit);
}
...
fn testPresentKeyTruncatedSkillCatalogStartup(alloc: Allocator, default_model: []const u8, default_agent_step_limit: usize) !app_lifecycle.StartupState {
    var state = try testPresentKeyNoContextStartup(alloc, default_model, default_agent_step_limit);
...
fn testPresentKeyNoContextStartup(alloc: Allocator, default_model: []const u8, default_agent_step_limit: usize) !app_lifecycle.StartupState {
    var state = try testPresentKeyStartup(alloc, default_model, default_agent_step_limit);
    state.context_enabled = false;
    return state;
}
