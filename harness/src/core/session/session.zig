...
/// The floor a compacted summary is allowed to shrink to, and what a session with
/// an unknown context window gets. This was the only budget before the window was
/// plumbed through, so an unconfigured runtime still behaves exactly as it did.
const compact_summary_max_chars: usize = 1200;
/// The ceiling, so a million-token window cannot spend a whole screen of summary
/// on a thread that only ever needed a paragraph.
const compact_summary_ceiling_chars: usize = 16000;
/// Share of the model's context window a compacted summary may occupy, in percent.
/// The compacted memory of a long session is worth about this much of the window;
/// past it the summary starts crowding out the recent turns it exists to support.
const compact_summary_window_percent: usize = 2;
/// Lines scale with the budget at roughly this many characters per line, so a
/// bigger budget buys more evidence rather than longer individual lines.
const compact_summary_chars_per_line: usize = 50;
...
/// model's usable context and a rounding error in a large one's. Everything is
/// derived from `context_window_tokens`, and a zero window (the default, and what
/// every caller passed before this existed) reproduces the old fixed numbers.
pub const CompactionBudget = struct {
    max_chars: usize = compact_summary_max_chars,
    max_lines: usize = compact_summary_max_lines,

    pub fn forWindow(context_window_tokens: usize) CompactionBudget {
        if (context_window_tokens == 0) return .{};
        const share = context_window_tokens * 4 * compact_summary_window_percent / 100;
        const chars = std.math.clamp(share, compact_summary_max_chars, compact_summary_ceiling_chars);
        return .{
            .max_chars = chars,
            .max_lines = @max(compact_summary_max_lines, chars / compact_summary_chars_per_line),
        };
    }

    /// How many entries one section of the summary may contribute.
    ///
    /// Sections used to carry hardcoded caps of 4/3/4, which is why a long coding
    /// session's file mutations and a long research session's fetches were both
    /// cut to the same three lines. Scaling with the budget lets the evidence a
    /// task actually produced be the thing that fills it.
    pub fn sectionCap(self: CompactionBudget, floor: usize) usize {
        return @max(floor, self.max_lines / 6);
    }
};
...
test "legacy image repair rewrites three repeated ordinals across persisted turn variants" {
    ...
    defer alloc.free(history[1].background_command.user.text);
    ...
    try std.testing.expectEqualStrings(
        "background [Image #2]",
        history[1].background_command.user.text,
    );
    try std.testing.expectEqualStrings(
        "interrupted [Image #3]",
...
fn compactedSummaryTurnForPrefix(
    ...
    if (cache) |entry| {
        ...
        if (summarizer != null and !built.model_written) {
            defer freeHistoryTurn(std.heap.c_allocator, .{ .compacted_summary = built.turn });
            const copy = try dupeHistoryTurn(alloc, .{ .compacted_summary = built.turn });
            return copy.compacted_summary;
        }
        entry.replace(prefix, built.turn, budget);
        owns_prefix = false;
        const copy = try dupeHistoryTurn(alloc, .{ .compacted_summary = entry.summary.? });
        return copy.compacted_summary;
    }
...
test "the original request survives repeated compaction" {
    ...
    const first_batch = [_]HistoryTurn{
        .{ .assistant = .{
    ...
    // Fold five more generations in. The goal must still be there, and must not
    // have been re-indented into oblivion by the nesting.
    var carried = first;
    var owned: ?CompactedSummaryHistoryTurn = null;
    defer if (owned) |entry| freeHistoryTurn(alloc, .{ .compacted_summary = entry });
    ...
    while (generation < 5) : (generation += 1) {
        ...
        owned = next;
        carried = next;
        try std.testing.expect(std.mem.indexOf(u8, next.summary, "port the parser") != null);
    }
}

test "compaction evidence keeps failures and file writes over a flood of reads" {
    const alloc = std.testing.allocator;

    // One early turn that reads six files, then later turns that write a file and
    // fail an edit. The old positional rule spent the whole section on the reads
    // and returned before ever reaching either.
    var reads: [6]core_types.FileEvidence = undefined;
    for (&reads) |*entry| {
        entry.* = .{
            .path = @constCast("src/read_only.zig"),
            .tool_call_id = @constCast("call-read"),
            .tool_name = @constCast("read_file"),
            .action = .read,
        };
    }
    var writes = [_]core_types.FileEvidence{.{
        .path = @constCast("src/written.zig"),
        .tool_call_id = @constCast("call-write"),
        .tool_name = @constCast("write_file"),
        .action = .write,
    }};
    var failed = [_]core_types.FileEvidence{.{
        .path = @constCast("src/broken.zig"),
        .tool_call_id = @constCast("call-fail"),
    ...
    const removed = [_]HistoryTurn{
        .{ .assistant = .{
            .user = .{ .text = @constCast("look around") },
            .assistant = @constCast("read the tree"),
            .execution = .{ .files = reads[0..] },
        } },
        .{ .assistant = .{
            .user = .{ .text = @constCast("apply the fix") },
            .assistant = @constCast("applied it"),
            .execution = .{ .files = writes[0..] },
        } },
        .{ .assistant = .{
            .user = .{ .text = @constCast("and the other one") },
            .assistant = @constCast("that one failed"),
            .execution = .{ .files = failed[0..] },
        } },
    };

    const summary = try buildCompactedSummaryText(alloc, null, &removed, .{});
    defer alloc.free(summary);

    try std.testing.expect(std.mem.indexOf(u8, summary, "src/broken.zig") != null);
    try std.testing.expect(std.mem.indexOf(u8, summary, "src/written.zig") != null);
}

test "a known context window scales the summary budget" {
    const unknown = CompactionBudget.forWindow(0);
    try std.testing.expectEqual(compact_summary_max_chars, unknown.max_chars);
    try std.testing.expectEqual(compact_summary_max_lines, unknown.max_lines);

    const small = CompactionBudget.forWindow(8_000);
    try std.testing.expectEqual(compact_summary_max_chars, small.max_chars);

    const large = CompactionBudget.forWindow(1_000_000);
    try std.testing.expectEqual(compact_summary_ceiling_chars, large.max_chars);
    try std.testing.expect(large.max_lines > unknown.max_lines);
    try std.testing.expect(large.sectionCap(4) > unknown.sectionCap(4));
}

test "the token trigger only speaks when the window is known" {
...
test "token pressure compacts a history that is under the turn limit" {
    ...
    while (index < 6) : (index += 1) {
        try history.append(alloc, try makeAssistantTurn(
            alloc,
            "a request long enough to matter for the token estimate",
            "a reply long enough to matter for the token estimate",
        ));
    }
...
test "a fallback summary is not cached, so the next compaction of the same prefix retries the model" {
    const alloc = std.testing.allocator;
    ...
    fake.fail = false;
    const second = try compactedSummaryTurnForPrefix(alloc, &removed, null, &removed, .{}, fake.handle(), null, &cache);
    defer freeHistoryTurn(alloc, .{ .compacted_summary = second });
    try std.testing.expectEqual(@as(usize, 2), fake.calls);
    try std.testing.expect(cache.summary != null);
    try std.testing.expect(std.mem.indexOf(u8, second.summary, "finish the tokenizer port") != null);

    const third = try compactedSummaryTurnForPrefix(alloc, &removed, null, &removed, .{}, fake.handle(), null, &cache);
    freeHistoryTurn(alloc, .{ .compacted_summary = third });
    try std.testing.expectEqual(@as(usize, 2), fake.calls);
}

const original_request_header = "- Original request:";

/// The pinned goal carried out of a previous summary, so it survives compaction
/// generation after generation.
///
/// Without this the standing goal decays: it enters the first summary as an
/// ordinary user line, gets indented one level deeper on every later compaction,
/// and eventually falls off the end of the budget — leaving a model that knows
/// what it was doing recently and not what it was asked for.
fn pinnedGoalLine(existing: ?CompactedSummaryHistoryTurn) ?[]const u8 {
    const entry = existing orelse return null;
    var iter = std.mem.splitScalar(u8, entry.summary, '\n');
    ...
    while (iter.next()) |line| {
        ...
        if (std.mem.eql(u8, trimmed, original_request_header)) found_header = true;
    }
    return null;
}
...
fn buildCompactedSummaryText(
    ...
    const carried_goal = pinnedGoalLine(existing);
    const total_compacted = (if (existing) |entry| entry.removed_turn_count else 0) + removed.len;

    try lines.append(arena, "Conversation summary:");
    try lines.append(arena, try std.fmt.allocPrint(arena, "- Earlier turns compacted: {d}", .{total_compacted}));

    // The goal leads, before anything that can be squeezed out by the budget.
    if (carried_goal) |goal| {
        try lines.append(arena, original_request_header);
        try lines.append(arena, try std.fmt.allocPrint(arena, "  - {s}", .{goal}));
    }

    try appendUserSummaryLines(arena, &lines, removed, budget, carried_goal == null);
    try appendAssistantSummaryLines(arena, &lines, removed, budget);
    try appendExecutionSummaryLines(arena, &lines, removed, budget);
    try appendBackgroundSummaryLines(arena, &lines, removed, budget);
    try appendInterruptedSummaryLines(arena, &lines, removed, budget);

    // Older context goes last: it is the most compressible thing here, and when the
    // budget runs out it is what should be dropped rather than this batch's evidence.
    if (existing) |entry| {
        var wrote_header = false;
        var existing_lines = std.mem.splitScalar(u8, entry.summary, '\n');
        var skipping_goal = false;
        while (existing_lines.next()) |line| {
            const trimmed = std.mem.trim(u8, line, " \t\r\n");
            if (trimmed.len == 0) continue;
            // The goal was already re-emitted at the top; do not nest a second copy.
            if (std.mem.eql(u8, trimmed, original_request_header)) {
                skipping_goal = true;
                continue;
            }
            if (skipping_goal) {
...
/// The user's requests, oldest-first but keeping the newest when the budget runs out.
///
/// The first request in a session is pinned: it is the standing goal, and every
/// later turn is only meaningful against it. Everything else is kept newest-first,
/// because scanning forward from the oldest — which is what this did — spends the
/// whole section on the opening of a long thread and drops the decisions that
/// actually describe where the work got to.
fn appendUserSummaryLines(
    arena: Allocator,
    lines: *std.ArrayList([]const u8),
    removed: []const HistoryTurn,
    budget: CompactionBudget,
    ...
    const cap = budget.sectionCap(4);
    var picked: std.ArrayList([]const u8) = .empty;
    defer picked.deinit(arena);

    // The goal, taken from the front, and only while it is still in this batch:
    // on later compactions it already lives in the folded-in previous summary.
    var goal: ?[]const u8 = null;
    if (is_first_compaction) {
        for (removed) |turn| {
            const text = userTurnText(turn) orelse continue;
            const summary = compactLineText(arena, text, compact_summary_max_line_chars - 4) catch continue;
    ...
    var index = removed.len;
    while (index > 0 and picked.items.len < cap) {
        index -= 1;
        const text = userTurnText(removed[index]) orelse continue;
        const summary = compactLineText(arena, text, compact_summary_max_line_chars - 4) catch continue;
        if (summary.len == 0) continue;
        if (goal) |pinned| if (std.mem.eql(u8, pinned, summary)) continue;
        try picked.append(arena, summary);
    }

    if (goal == null and picked.items.len == 0) return;

    if (goal) |pinned| {
        try lines.append(arena, "- Original request:");
        try lines.append(arena, try std.fmt.allocPrint(arena, "  - {s}", .{pinned}));
    }

    if (picked.items.len == 0) return;
    try lines.append(arena, "- Recent user requests:");
    var back = picked.items.len;
    while (back > 0) {
        back -= 1;
        try lines.append(arena, try std.fmt.allocPrint(arena, "  - {s}", .{picked.items[back]}));
    }
}

/// What the assistant actually concluded, newest-first for the same reason.
fn appendAssistantSummaryLines(
    arena: Allocator,
    lines: *std.ArrayList([]const u8),
    removed: []const HistoryTurn,
    budget: CompactionBudget,
) !void {
    const cap = budget.sectionCap(3);
    var picked: std.ArrayList([]const u8) = .empty;
    defer picked.deinit(arena);
    ...
    while (index > 0 and picked.items.len < cap) {
        index -= 1;
        const assistant_text = switch (removed[index]) {
...
/// What survives of a turn's tool activity, ranked so the budget buys the evidence
/// that explains the state of the work rather than whatever happened to run first.
///
/// The old rule was positional — four results and `return` — so a single turn that
/// opened four files consumed the whole section and the failed build three turns
/// later never appeared. The ranks below are what a resumed session actually needs
/// to not repeat itself: what broke, what it changed, then everything else.
const EvidenceRank = enum(u8) {
    /// A tool that failed. Without this a resumed session retries the failure.
    failure = 0,
    /// A file this session created, changed or removed: the work product so far.
    mutation = 1,
    /// Reads, searches, listings — recoverable by doing them again.
    observation = 2,
};

const RankedEvidence = struct {
    rank: EvidenceRank,
    /// Position in the removed batch; higher is more recent.
    order: usize,
    text: []const u8,
};

fn mutatingFileAction(action: core_types.FileEvidenceAction) bool {
    return switch (action) {
        .write, .edit, .delete, .rename, .copy => true,
        .read, .search, .list, .unknown => false,
    };
}

fn evidenceLessThan(_: void, a: RankedEvidence, b: RankedEvidence) bool {
    if (a.rank != b.rank) return @intFromEnum(a.rank) < @intFromEnum(b.rank);
    return a.order > b.order;
}

fn appendExecutionSummaryLines(
    arena: Allocator,
    lines: *std.ArrayList([]const u8),
    removed: []const HistoryTurn,
    budget: CompactionBudget,
) !void {
    var ranked: std.ArrayList(RankedEvidence) = .empty;
    defer ranked.deinit(arena);

    for (removed, 0..) |turn, order| {
        ...
        for (execution.tool_steps) |step| {
            for (step.tool_results) |result| {
                try ranked.append(arena, .{
                    .rank = if (result.status == .failure) .failure else .observation,
                    .order = order,
                    .text = try formatToolResultEvidenceLine(arena, result),
                });
            }
        }
        ...
        for (execution.files) |file| {
            ...
            const rank: EvidenceRank = if (file.status == .failure)
                .failure
            else if (mutatingFileAction(file.action))
                .mutation
            else
                .observation;
            try ranked.append(arena, .{
                ...
                .text = try std.fmt.allocPrint(
                    arena,
                    "  - file {s}: {s}{s}",
                    .{ @tagName(file.action), file.path, stale },
                ),
            });
        }
    }
    ...
    std.mem.sort(RankedEvidence, ranked.items, {}, evidenceLessThan);

    const cap = @min(ranked.items.len, budget.sectionCap(4));
    try lines.append(arena, "- Tool execution evidence:");
    for (ranked.items[0..cap]) |entry| try lines.append(arena, entry.text);
}
...
fn appendBackgroundSummaryLines(arena: Allocator, lines: *std.ArrayList([]const u8), removed: []const HistoryTurn, budget: CompactionBudget) !void {
    const cap = budget.sectionCap(3);
    var added: usize = 0;
...
fn appendInterruptedSummaryLines(arena: Allocator, lines: *std.ArrayList([]const u8), removed: []const HistoryTurn, budget: CompactionBudget) !void {
    const cap = budget.sectionCap(3);
    var added: usize = 0;
    var saw_header = false;
    for (removed) |turn| {
...
fn compressSummaryLines(alloc: Allocator, lines: []const []const u8, budget: CompactionBudget) ![]u8 {
    const max_lines = budget.max_lines;
    const max_chars = budget.max_chars;
    var out: std.Io.Writer.Allocating = .init(alloc);
    defer out.deinit();
    ...
    for (lines) |line| {
        ...
        if (line_count >= max_lines or char_count + candidate_chars > max_chars) {
            omitted += 1;
            continue;
        }
    ...
    if (omitted > 0 and line_count < max_lines) {
        ...
        if (char_count + candidate_chars <= max_chars) {
            if (line_count > 0) try out.writer.writeByte('\n');
...
test "appendHistoryMessages frees owned projection text when append fails" {
    ...
    var bg_failing = std.testing.FailingAllocator.init(std.testing.allocator, .{});
    const bg_alloc = bg_failing.allocator();
    var bg_messages: std.ArrayList(message.Message) = .empty;
    defer bg_messages.deinit(bg_alloc);
    try bg_messages.ensureTotalCapacity(bg_alloc, 1);
    bg_failing.fail_index = bg_failing.alloc_index + 1;
    bg_failing.resize_fail_index = bg_failing.resize_index;
    const bg = [_]HistoryTurn{.{ .background_command = .{ .user = .{ .text = @constCast("run") }, .log_path = @constCast("/tmp/log"), .expect_url = true } }};
    try std.testing.expectError(error.OutOfMemory, appendHistoryMessages(bg_alloc, &bg_messages, &bg));
    for (bg_messages.items) |*msg| msg.deinit(bg_alloc);
}
...
test "resume projection emits compacted summary before background command context" {
    const alloc = std.testing.allocator;

    const history = [_]HistoryTurn{
        .{ .compacted_summary = .{ .summary = @constCast("summary"), .removed_turn_count = 2, .compaction_count = 1 } },
        .{ .background_command = .{ .user = .{ .text = @constCast("run dev") }, .log_path = @constCast("/tmp/server.log"), .expect_url = true, .url = @constCast("http://localhost:3000") } },
    };
    var messages: std.ArrayList(message.Message) = .empty;
    defer {
        for (messages.items) |*msg| msg.deinit(alloc);
    ...
    try std.testing.expectEqual(.user, messages.items[1].role);
...
test "specialized history replays execution before visible terminal text and context" {
    const alloc = std.testing.allocator;
    var calls = [_]ToolCall{.{
        .id = "call_read",
        .name = "read_file",
    ...
    const execution = ExecutionMemory{ .tool_steps = steps[0..] };
    const history = [_]HistoryTurn{
        .{ .background_command = .{
    ...
    try std.testing.expectEqual(@as(usize, 10), messages.items.len);
    try std.testing.expectEqualStrings("run dev", messages.items[0].content.?.asText());
    try std.testing.expectEqualStrings("call_read", messages.items[1].tool_calls[0].id);
    try std.testing.expectEqualStrings("file contents", messages.items[2].content.?.asText());
    try std.testing.expectEqualStrings("The server is starting.", messages.items[3].content.?.asText());
    try std.testing.expectEqual(.user, messages.items[4].role);
    try std.testing.expectEqualStrings("inspect", messages.items[5].content.?.asText());
    try std.testing.expectEqualStrings("call_read", messages.items[6].tool_calls[0].id);
    try std.testing.expectEqualStrings("file contents", messages.items[7].content.?.asText());
    try std.testing.expect(std.mem.startsWith(
        u8,
        messages.items[8].content.?.asText(),
        "I inspected the entry point.",
    ));
    try std.testing.expectEqual(.user, messages.items[9].role);
}
...
test "budgeted Message and Chat projections retain latest turn and identical trimmed evidence" {
    ...
    const history = [_]HistoryTurn{
        ...
        .{ .background_command = .{
            ...
            .log_path = @constCast("/tmp/retained.log"),
            .expect_url = true,
    ...
    for (messages.items, chat_messages.items) |projected, chat| {
        ...
        if (projected.content) |content| {
            ...
            saw_background = saw_background or
                std.mem.find(u8, content.asText(), "/tmp/retained.log") != null;
...
test "SessionRuntime.appendHistoryEntry stores exact deep copies for all variants" {
    const alloc = std.testing.allocator;
    var runtime: SessionRuntime = .{ .max_history_turns = 0 };
    defer runtime.deinit(alloc);

    var caller_assistant = try makeAssistantTurn(alloc, "alpha", "assistant alpha");
    defer freeHistoryTurn(alloc, caller_assistant);
    var caller_background = HistoryTurn{ .background_command = .{
        .user = .{ .text = try alloc.dupe(u8, "run server") },
        .assistant = try alloc.dupe(u8, "server ready"),
        .execution = .{ .files = blk: {
            const files = try alloc.alloc(FileEvidence, 1);
            files[0] = .{
                .path = try alloc.dupe(u8, "src/main.zig"),
                .tool_call_id = try alloc.dupe(u8, "call_read"),
        ...
        .log_path = try alloc.dupe(u8, "/tmp/server.log"),
        .expect_url = true,
        .url = try alloc.dupe(u8, "http://localhost:3000"),
    } };
    defer freeHistoryTurn(alloc, caller_background);
    var caller_summary = HistoryTurn{ .compacted_summary = .{
        .summary = try alloc.dupe(u8, "prior summary"),
        .removed_turn_count = 4,
        .compaction_count = 2,
    } };
    defer freeHistoryTurn(alloc, caller_summary);

    try runtime.appendHistoryEntry(alloc, caller_assistant);
    try runtime.appendHistoryEntry(alloc, caller_background);
    try runtime.appendHistoryEntry(alloc, caller_summary);

    caller_assistant.assistant.user.text[0] = 'z';
    caller_background.background_command.assistant.?[0] = '!';
    caller_background.background_command.execution.files[0].path[0] = '!';
    caller_background.background_command.log_path[0] = '!';
    ...
    defer freeHistoryTurn(alloc, extra);
...
test "appendHistoryTurnToOwnedContext moves old ownership, duplicates new turn, and compacts" {
    ...
    const next = try appendHistoryTurnToOwnedContext(alloc, current, caller_turn, 2, 0);
...
test "appendHistoryTurnToOwnedContext frees duplicate fields on allocation failure" {
    ...
    try std.testing.expectError(error.OutOfMemory, appendHistoryTurnToOwnedContext(failing.allocator(), &.{}, turn, 0, 0));
}
...
test "SessionRuntime.appendHistoryMessages matches top-level projection and preserves ownership flags" {
    ...
    const history = [_]HistoryTurn{
        ...
        .{ .background_command = .{
            .user = .{ .text = @constCast("run dev") },
            .log_path = @constCast("/tmp/dev.log"),
            .expect_url = true,
            .url = @constCast("http://localhost:3000"),
        } },
    };
    ...
    try std.testing.expect(static_messages.items[4].owns_content);
}
...
test "history context formatters return exact text" {
    ...
    const with_url = try formatBackgroundHistoryContext(alloc, .{
        .user = .{ .text = @constCast("run") },
        .log_path = @constCast("/tmp/url.log"),
        .expect_url = true,
        .url = @constCast("http://localhost:3000"),
    });
    ...
    const pending = try formatBackgroundHistoryContext(alloc, .{
        .user = .{ .text = @constCast("run") },
        .log_path = @constCast("/tmp/pending.log"),
        .expect_url = true,
    });
    ...
    const command = try formatBackgroundHistoryContext(alloc, .{
        .user = .{ .text = @constCast("run") },
        .log_path = @constCast("/tmp/command.log"),
        .expect_url = false,
    });
