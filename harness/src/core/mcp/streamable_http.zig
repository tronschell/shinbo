...
fn isLegacyDiscoveryCompatibilityError(object: std.json.ObjectMap) bool {
    ...
    return code.integer == legacy_missing_session_header_code and
        std.mem.eql(
            u8,
            message.string,
...
test "modern MCP JSON responses are bounded and retain request ownership" {
    ...
    var unrelated_missing_id_error_reader = std.Io.Reader.fixed(unrelated_missing_id_error);
    try std.testing.expectError(
        error.InvalidJsonResponse,
        readJsonResponse(
            alloc,
            &unrelated_missing_id_error_reader,
            7,
