...
pub fn onStreamReasoningChunk(ctx: *anyopaque, chunk: []const u8) void {
    ...
    // Forwarded raw: reasoning is not markdown-rendered like the answer is, and a
    // surface that has nowhere to put it drops the emission.
    stream_ctx.hooks.push_text(stream_ctx.hooks.ctx, .{ .reasoning = chunk }) catch {};
}
...
const StreamCapture = struct {
    ...
    fn captureText(raw: *anyopaque, emission: runtime_deps.TextEmission) !void {
        ...
        const text = switch (emission) {
            .assistant_source, .reasoning => unreachable,
