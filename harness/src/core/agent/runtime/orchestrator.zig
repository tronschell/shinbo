...
fn normalize_terminal_request_tool_calls(
    ...
    for (source, 0..) |call, index| {
        ...
        const arguments_json = try tool_args.normalizedTerminalArguments(
            alloc,
            call.arguments_json,
        ) orelse continue;
        if (normalized == null) {
...
test "terminal request normalization unwraps only exact native terminal calls" {
    ...
    const browser = try normalize_terminal_request_tool_calls(arena, browser_registry, &calls);
    ...
    const non_exact = try normalize_terminal_request_tool_calls(arena, native_registry, &non_exact_calls);
    ...
    const malformed = try normalize_terminal_request_tool_calls(arena, native_registry, &malformed_calls);
...
fn check_terminal_request_normalization_allocation_failures(alloc: Allocator) !void {
    ...
    const normalized = try normalize_terminal_request_tool_calls(alloc, registry, &source);
...
) context_experiments.Settings {
    var settings = config.context_experiments;
    if (settings.context_window_tokens == 0) {
        if (capabilities.context_window) |window| settings.context_window_tokens = @intCast(window);
    }
    return settings;
}

test "experiment settings fall back to the model's own context window" {
    const cancel = try std.testing.allocator.create(std.atomic.Value(bool));
    defer std.testing.allocator.destroy(cancel);
    ...
    const config: Config = .{
        ...
        .agent_step_limit = 8,
        .cancel_flag = cancel,
        .context_experiments = .{ .prune_tools_percent = 60 },
    };
    try std.testing.expectEqual(
        @as(usize, 256_000),
        experimentSettings(config, .{ .context_window = 256_000 }).context_window_tokens,
    );
    var pinned = config;
    pinned.context_experiments.context_window_tokens = 1_000_000;
    try std.testing.expectEqual(
        @as(usize, 1_000_000),
        experimentSettings(pinned, .{ .context_window = 256_000 }).context_window_tokens,
    );
    try std.testing.expectEqual(
        @as(usize, 0),
        ...
        while (true) {
            ...
            if (experiment.changedAnything()) {
                debug_trace.eventf(
                    ...
                    "context_experiment",
                    step_ctx,
                    "step_index={d} estimated_tokens={d} percent_used={d} pruned_results={d} reinjected={} saved_tokens={d} added_tokens={d}",
                    .{ current_step_index, experiment.estimated_tokens, experiment.percent_used, experiment.pruned_results, experiment.reinjected, experiment.saved_tokens, experiment.added_tokens },
                );
                if (deps.push_context_experiment) |push| {
                    try push(deps.ctx, experiment);
                }
            }
