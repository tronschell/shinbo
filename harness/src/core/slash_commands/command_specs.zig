...
test "slash completion aliases participate in ranked order" {
    try std.testing.expectEqualStrings("/continue", firstSlashCompletion(testSlashRegistry(), "/co").?);
    try std.testing.expectEqual(@as(usize, 4), slashCompletionCount(testSlashRegistry(), "/co"));
    try std.testing.expectEqualStrings("/cost", nthSlashCompletion(testSlashRegistry(), "/co", 1).?);
    try std.testing.expectEqualStrings("/copy", nthSlashCompletion(testSlashRegistry(), "/co", 2).?);
    try std.testing.expectEqualStrings("/cost", firstSlashCompletion(testSlashRegistry(), "/cos").?);
}
