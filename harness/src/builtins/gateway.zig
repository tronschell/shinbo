...
const perplexity_search_backend_id = web_search_contract.SearchBackendId{ .value = "perplexity_search" };
const parallel_search_backend_id = web_search_contract.SearchBackendId{ .value = "parallel_search" };
...
pub fn preferredWebSearchBackendsOverride(raw: ?[]const u8) !?[]const web_search_contract.SearchBackendId {
    ...
    if (std.mem.eql(u8, value, "perplexity_search")) return &perplexity_search_backend;
    if (std.mem.eql(u8, value, "parallel_search")) return &parallel_search_backend;
...
test "built-in gateway defaults preserve active provider policy" {
    try std.testing.expectEqualStrings("nvidia/nemotron-3-super-120b-a12b:free", default_model);
...
fn stubFetchForbiddenCredits(
    ...
    return .{
        .status = .forbidden,
        .body = try alloc.dupe(u8, "{\"error\":{\"code\":\"credit_card_required\",\"message\":\"Buy credits to use this provider.\"}}"),
    };
}
...
test "built-in model catalog owns default and loopback target resolution" {
    ...
    try std.testing.expectEqualStrings("https://openrouter.ai/api/v1/models", default_url);
    ...
    try std.testing.expectEqualStrings("https://openrouter.ai/api/v1/models", rejected_url);
}
...
test "built-in gateway web search override selects one backend and rejects unknown values" {
    ...
    try std.testing.expect(perplexity_search_backend_id.eql((try preferredWebSearchBackendsOverride("perplexity_search")).?[0]));
    try std.testing.expect(parallel_search_backend_id.eql((try preferredWebSearchBackendsOverride("parallel_search")).?[0]));
    try std.testing.expectError(error.InvalidWebSearchBackend, preferredWebSearchBackendsOverride("ai_gateway_parallel_search"));
}
...
fn fetchModelCatalogResponse(
    ...
    return if (cancel_flag) |flag|
        gateway_client.fetchGatewayJsonCancellable(alloc, api_key, model_catalog_url, flag)
    else
        gateway_client.fetchGatewayJson(alloc, api_key, model_catalog_url);
}
