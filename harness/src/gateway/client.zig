...
test "gateway extra headers derive session identity and affinity together" {
    ...
    for (cases) |case| {
        const headers = gatewayExtraHeaders(&buf, "test/model", case.session_id);
...
pub fn findHeaderValue(head: std.http.Client.Response.Head, name: []const u8) ?[]const u8 {
...
fn parseSseTokenDetail(usage_value: std.json.Value, key: []const u8, detail: []const u8) ?u64 {
    if (usage_value != .object) return null;
    const token_value = usage_value.object.get(key) orelse return null;
    if (token_value != .object) return null;
    const detail_value = token_value.object.get(detail) orelse return null;
    if (detail_value != .integer or detail_value.integer < 0) return null;
    return @intCast(detail_value.integer);
}
...
fn parseSseBilling(
    ...
    return .{
        ...
        .cost_micro_usd = types.parseMicroDollars(cost_value.string),
...
test "consumeSseStream captures exact terminal billing" {
    ...
    try std.testing.expectEqual(@as(?u64, 20), completion.usage.cache_read_tokens);
    try std.testing.expectEqual(@as(?u64, 10), completion.usage.cache_write_tokens);
    try std.testing.expectEqual(@as(?u64, 12_300), billing.cost_micro_usd);
}
...
test "E2E gateway URL override accepts loopback HTTP only" {
    try std.testing.expectEqualStrings(
        ...
        try selectE2eGatewayUrl(null, "https://openrouter.ai/api/v1/chat/completions"),
    );
    try std.testing.expectEqualStrings(
        "http://127.0.0.1:43123/v3/ai/language-model",
        try selectE2eGatewayUrl("http://127.0.0.1:43123/v3/ai/language-model", "https://openrouter.ai/api/v1/chat/completions"),
    );
    try std.testing.expectEqualStrings(
        "http://[::1]:43123/v1/models",
        try selectE2eGatewayUrl("http://[::1]:43123/v1/models", "https://openrouter.ai/api/v1/models"),
    );
    try std.testing.expectError(
        error.InvalidE2EGatewayUrl,
        selectE2eGatewayUrl("https://openrouter.ai/api/v1/chat/completions", "https://openrouter.ai/api/v1/chat/completions"),
    );
    try std.testing.expectError(
        error.InvalidE2EGatewayUrl,
        selectE2eGatewayUrl("http://127.0.0.1:43123@openrouter.ai/api/v1/chat/completions", "https://openrouter.ai/api/v1/chat/completions"),
    );
}
...
test "findResolvedModelHeader reads the resolved model response header" {
    ...
    try std.testing.expectEqualStrings("ai-language-model-id", header.name);
...
fn expectCancellableGatewayJsonCancellation(
    ...
    const result = if (use_tls)
        ...
        fetchGatewayJsonCancellable(std.testing.allocator, api_key, "https://openrouter.ai/api/v1/models", &cancel_flag);
