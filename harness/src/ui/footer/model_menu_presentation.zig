...
fn loadedCatalogStatusText(state: model_cache_runtime.ModelMenuCatalogState) ?[]const u8 {
    ...
    if (state.access_level == .authenticated) {
        return "Model catalog: authenticated with an API key.";
    }
    return null;
}
...
fn retryableFailureText(failure: ?model_cache_runtime.ModelMenuCatalogState.Failure) ?[]const u8 {
    ...
    return switch (value.category) {
        .rate_limited => "The provider rate limited model discovery; retry /models.",
        .transport, .gateway_unavailable => "Could not reach the provider; retry /models.",
...
test "model menu states and navigation budget stay bounded" {
    ...
    try std.testing.expect(std.mem.find(u8, failed_state.items, "Could not reach the provider; retry /models.") != null);
...
test "model menu status follows provenance and retryable failure precedence" {
    ...
    const cases = [_]struct {
        ...
        .{ .state = .{ .access_level = .public_only, .public_only_reason = .no_credential, .private_models_hidden = true, .failure = .{ .category = .rate_limited, .retryable = true } }, .expected = "The provider rate limited model discovery; retry /models." },
        .{ .state = .{ .access_level = .authenticated, .failure = .{ .category = .rate_limited, .retryable = true } }, .expected = "The provider rate limited model discovery; retry /models." },
