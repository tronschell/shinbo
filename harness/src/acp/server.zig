...
pub fn releaseActiveSession(state: *ServerState) !void {
    ...
    if (comptime !host_target.is_wasm) {
        ...
        flushActiveSessionUsage(state) catch |err| {
            active.session_rt.usage.startReconciliation(state.alloc, state.api_key);
            return err;
        };
...
fn dispatch(state: *ServerState, alloc: Allocator, msg: *jsonrpc.Message) !void {
    ...
    return switch (method) {
        ...
        .session_prompt => startPrompt(state, alloc, msg),
        .session_compact => prompt_handler.handleCompact(state, alloc, msg),
        .session_set_config_option => handleSetConfigOption(state, alloc, msg),
        .session_set_mode => handleSetMode(state, alloc, msg),
        .initialize,
        .session_cancel,
...
fn publishPromptOutcome(active: *ActivePrompt, outcome: prompt_handler.TerminalOutcome) !void {
    switch (outcome) {
        .stop_reason => |terminal| {
            ...
            try acp_types.writePromptResponse(&response.writer, terminal.reason, terminal.usage);
...
fn handleSetConfigOption(state: *ServerState, alloc: Allocator, msg: *jsonrpc.Message) !void {
    ...
    if (std.mem.eql(u8, config_id, "model")) {
        ...
        const window = std.fmt.parseInt(usize, value, 10) catch
            return state.writer.writeError(alloc, msg.id, .{
                .code = ErrorCode.invalid_params,
                .message = "Invalid context_window",
            });
        const session = if (state.active_session) |*active| active else return state.writer.writeError(alloc, msg.id, .{
            .code = ErrorCode.invalid_request,
            .message = "No active session",
        ...
        session.context_experiments = parseContextExperiments(value) catch
            return state.writer.writeError(alloc, msg.id, .{
                .code = ErrorCode.invalid_params,
...
fn parseContextExperiments(value: []const u8) !context_experiments.Settings {
    ...
    while (pairs.next()) |raw| {
        ...
        } else if (std.mem.eql(u8, key, "prune_steps")) {
...
test "context experiment options parse into the settings the loop reads" {
    ...
    const unknown = try parseContextExperiments("something_else=3");
...
fn handleSteerChild(state: *ServerState, alloc: Allocator, msg: *jsonrpc.Message) !void {
    ...
    var result = host.sendMessage(alloc, .{
        ...
        .code = ErrorCode.internal_error,
        .message = "The subagent store refused the message",
    });
...
test "ACP method parser classifies request dispatch methods" {
    ...
    try std.testing.expectEqual(AcpMethod.session_compact, AcpMethod.parse("session/compact"));
