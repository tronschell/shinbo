...
description: How to build things for yourself inside Shinbo's own repository — a new interface, main-process behaviour, a skill, a UI plugin, a scheduled job — with the standards to hold to, the commands that build and check each layer, how to launch a second dev instance without killing the Shinbo you are running in, and how to verify the change actually landed. Use whenever the work changes Shinbo's own code, harness, or interface rather than the user's files.
---
...
Everything here needs the Shinbo checkout connected as this thread's folder. If
...
2. **A component of your own** — the `component` tool. This is what "build
   ...
   **Settings → Built by Shinbo**, which also shows each one, switches it off,
...
`No handler registered for 'shinbo:…'` errors in the window. Those are a stale
...
**Do not quit the Shinbo you are running in, and do not ask the user to.** Launch a
...
separate `--user-data-dir` is what avoids that. `SHINBO_DATA_DIR` keeps the dev
...
- **Drive the real IPC:** `node desktop/scripts/drive.mjs '<statements>' [ms]`
  runs against `window.shinbo` in the running window, which is the same path a
  click takes. The argument is an async function *body*, so it needs `return`;
  set `SHINBO_CDP_PORT=9223` to point it at your dev instance:

  ```sh
  SHINBO_CDP_PORT=9223 node desktop/scripts/drive.mjs \
    'return await window.shinbo.searchImportedSkills({ query: "build", limit: 8 })'
  ```
- **Look at it:** `node desktop/scripts/shot.mjs 9223 /tmp/shinbo-shots 1440 900`
