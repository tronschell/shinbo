...
import { applyNoteTags, keepNote, noteInVault, notesRoot, saveVault } from "../main/vault";
...
test("S4 background tagging preserves edits made in Obsidian while the tagger was running", async (t) => {
  ...
  const tag = lifted("tagKeptNote", { readFileSync: fs.readFileSync, tagNote: () => tagging, tagger: {}, applyNoteTags, fireEvent() {}, notesChanged() {}, console: { warn() {}, error() {} } });
...
test("S4 an untouched newly kept note still receives its automatic title and tags", async (t) => {
  ...
  const note = await keepNote(vault, { kind: "note", title: "Draft", text: "My original note" });
  const events: string[] = [];
  const tag = lifted("tagKeptNote", { readFileSync: fs.readFileSync, tagNote: async () => ({ title: "Useful title", tags: ["project"] }), tagger: {}, applyNoteTags, fireEvent: (name: string) => events.push(name), notesChanged() {}, console });
