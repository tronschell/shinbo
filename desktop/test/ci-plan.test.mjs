...
test("trust boundaries, harness code, deletions and unknown files retain all checks", () => {
  for (const file of ["harness/src/main.zig", "crates/core/src/lib.rs", "desktop/main/harness.ts", "desktop/shared/bridge.ts", "desktop/test/harness.test.ts", "desktop/skills/building-emma/SKILL.md", "new-build-config", "desktop/src-old/removed.ts"]) {
...
test("promotion requires the same repository's dev tree and a newer stable version", () => {
  const event = { repository: { full_name: "owner/emma" }, pull_request: { head: { ref: "dev", repo: { full_name: "owner/emma" } } } };
  ...
  assert.throws(() => validatePromotion({ ...event, pull_request: { head: { ref: "dev", repo: { full_name: "fork/emma" } } } }, "0.6.9", "0.7.0", "tree", "tree"));
});

test("published versions skip builds while release lookup failures stop CI", async () => {
  assert.equal(await releaseExists("owner/emma", "0.6.0", async () => ({ status: 200 })), true);
  assert.equal(await releaseExists("owner/emma", "0.6.0", async () => ({ status: 404 })), false);
  for (const status of [401, 403, 429, 500]) await assert.rejects(releaseExists("owner/emma", "0.6.0", async () => ({ status })));
  await assert.rejects(releaseExists("owner/emma", "0.6.0", async () => { throw new Error("network failure"); }));
});
